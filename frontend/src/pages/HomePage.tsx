...
export function HomePage({ token, host }: { token: string | null; host: string }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-cyan-500/20 via-blue-500/10 to-purple-500/10 p-4 sm:p-6 ring-1 ring-white/10">
        <p className="text-xs sm:text-sm uppercase tracking-[0.18em] text-cyan-200">Host</p>
        <p className="text-2xl sm:text-3xl font-semibold text-white">{host || 'Unknown'}</p>
        <p className="text-xs sm:text-sm text-slate-200">SMB monitoring with automated blocking</p>
      </div>
      ...
      {stats ? (
        <>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 lg:grid-cols-2">
            ...
            <div className="flex items-center gap-3 rounded-xl bg-slate-900/50 p-3 sm:p-4 ring-1 ring-white/5">
              <AlarmClock className="text-cyan-300 flex-shrink-0" size={20} />
              <div className="text-xs sm:text-sm text-slate-300 min-w-0">
                <p className="font-semibold text-white">Timeline window</p>
                <p className="truncate">Change the window to view attempts over time.</p>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-xs sm:text-sm text-slate-300">
