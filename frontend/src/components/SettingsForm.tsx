...
export function SettingsForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl bg-card/70 p-4 sm:p-5 ring-1 ring-white/5 shadow-lg shadow-black/20">
      <div className="mb-4 flex items-start sm:items-center gap-3 text-slate-200">
        <Settings className="text-cyan-300 flex-shrink-0" size={20} />
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Settings</p>
          <p className="font-semibold text-white text-sm sm:text-base">Blocker parameters</p>
        </div>
      </div>
      {message && <p className="mb-2 rounded-lg bg-emerald-500/10 px-3 py-2 text-xs sm:text-sm text-emerald-200">{message}</p>}
      {error && <p className="mb-2 rounded-lg bg-rose-500/10 px-3 py-2 text-xs sm:text-sm text-rose-200">{error}</p>}
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
        <Field label="Attempt threshold">
          <input
            ...
            className="w-full rounded-lg bg-slate-900/60 px-3 py-2 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-400"
          />
        </Field>
      ...
      <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:gap-3">
        <button
          disabled={saving}
          className="flex items-center justify-center sm:justify-start gap-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-2 text-sm sm:text-base font-semibold text-white shadow-cyan-500/20 transition hover:from-cyan-400 hover:to-blue-400 disabled:opacity-60"
        >
        ...
        <button
          ...
          onClick={onScan}
          className="rounded-lg bg-slate-800/70 px-4 py-2 text-xs sm:text-sm text-slate-200 ring-1 ring-white/10 transition hover:bg-slate-700 disabled:opacity-60"
        >
          {running ? (
      ...
      <div className="mt-3 flex flex-col sm:flex-row sm:items-start gap-3 rounded-xl bg-slate-900/50 p-3 sm:p-4 text-xs sm:text-sm text-slate-200 ring-1 ring-white/10">
        <ScanEye className="text-cyan-300 flex-shrink-0" size={20} />
        <div className="min-w-0">
